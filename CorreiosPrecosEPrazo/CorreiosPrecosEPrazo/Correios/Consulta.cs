...
namespace CorreiosPrecosEPrazo.Correios
{
    class Consulta
    {
        ...
        public static cResultado ConsultarPrecosEPrazos(string nCdEmpresa, string sDsSenha, string nCdServico, string sCepOrigem, string sCepDestino, string nVlPeso, int nCdFormato, decimal nVlComprimento, decimal nVlAltura, decimal nVlLargura, decimal nVlDiametro, string sCdMaoPropria, decimal nVlValorDeclarado, string sCdAvisoRecebimento)
        {
            ...
            if(nVlComprimento < 16 || nVlComprimento > 100)
            {
                MessageBox.Show("O comprimento da embalegem tem que estar entre 16 cm e 100 cm");
                return null;
            }

            if (nVlLargura < 11 || nVlLargura > 100)
            {
                MessageBox.Show("A largura da embalegem tem que estar entre 11 cm e 100 cm");
                return null;
            }

            if (nVlAltura < 2 || nVlAltura > 100)
            {
                MessageBox.Show("A altura da embalegem tem que estar entre 2 cm e 100 cm");
                return null;
            }
            ...
            {
                decimal ValorMinimoValorDeclaradoAceito = System.Convert.ToDecimal(ConfiguracoesExcel.ValorPorteAte20g) * 10;
                if (nVlValorDeclarado < ValorMinimoValorDeclaradoAceito)
                {
                    MessageBox.Show(string.Format("O valor declarado precisa ser maior que {0}", Helpers.returnDinheiro(ValorMinimoValorDeclaradoAceito.ToString())));
                    return null;
                }
            }
